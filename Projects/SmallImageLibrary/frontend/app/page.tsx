...
import { fetchFacets, searchMedia } from '../src/api';
import { highlightSnippet } from '../src/highlight';
import { FacetsResponse, SearchResponse, SortMode } from '../src/types';
    ...
    <main className="min-h-screen bg-panel">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-2 border-b border-line pb-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-3xl font-semibold tracking-normal text-ink">Small Image Library</h1>
            <p className="mt-1 max-w-3xl text-sm text-slate-600">
              Service-backed search over noisy media metadata with weighted relevance, filters, Postgres storage, and Redis metrics.
            </p>
          </div>
        ...
        <section className="grid gap-4 rounded border border-line bg-white p-4 shadow-sm lg:grid-cols-[minmax(0,1fr)_220px_180px]">
          ...
          <button
            className="h-11 self-end rounded border border-line bg-white px-4 text-sm font-semibold text-ink hover:border-berry focus:outline-none focus:ring-2 focus:ring-berry/30"
            onClick={() => {
              setQuery('');
              setCredit('');
              setDateFrom('');
        ...
        {status === 'error' && (
          <section className="rounded border border-berry bg-white p-5 text-berry" role="alert">
            Search failed. Confirm the Docker API service is running on port 3001.
          </section>
        )}
